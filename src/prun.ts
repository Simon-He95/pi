...
export function printPrunInit(args: string[] = []) {
  ...
  if (shell === 'zsh') {
    script = [
      ...
      '  fc -R',
      '  if [[ $hint_cmd != pfind* && $hint_cmd != prun* && $hint_cmd != pi && $hint_cmd != pi\\ * && $hint_cmd != pil && $hint_cmd != pil\\ * && $hint_cmd != npm\\ install* && $hint_cmd != pnpm\\ add* && $hint_cmd != pnpm\\ install* && $hint_cmd != yarn\\ add* && $hint_cmd != yarn\\ install* && $hint_cmd != bun\\ add* && $hint_cmd != bun\\ install* ]]; then',
      '    return',
      '  fi',
      '  local last_line',
      ...
      '    return',
      '  fi',
      '  if [[ $last_cmd == prun || $last_cmd == prun\\ * || $last_cmd == pfind || $last_cmd == pfind\\ * || $last_cmd == pi || $last_cmd == pi\\ * || $last_cmd == pil || $last_cmd == pil\\ * ]]; then',
      '    local last_num',
    ...
    script = [
      ...
      '      if string match -q "pfind*" -- "$last_cmd"; or string match -q "prun*" -- "$last_cmd"; or string match -q "pi" -- "$last_cmd"; or string match -q "pi *" -- "$last_cmd"; or string match -q "pil" -- "$last_cmd"; or string match -q "pil *" -- "$last_cmd"; or string match -q "npm install*" -- "$last_cmd"; or string match -q "pnpm add*" -- "$last_cmd"; or string match -q "pnpm install*" -- "$last_cmd"; or string match -q "yarn add*" -- "$last_cmd"; or string match -q "yarn install*" -- "$last_cmd"; or string match -q "bun add*" -- "$last_cmd"; or string match -q "bun install*" -- "$last_cmd"',
    ...
    script = [
      ...
      '$script:__PRUN_HISTORY_READY = $false',
      '',
      ...
      '    return',
      '  }',
      '  $firstSync = -not $script:__PRUN_HISTORY_READY',
      '  $script:__PRUN_HISTORY_READY = $true',
      ...
      '    $hintKey = $hintCmd',
      '  }',
      '  if ($hintKey -and $script:__PRUN_HISTORY_HINT_TS -eq $hintKey) {',
      '    return',
      '  }',
      '  $script:__PRUN_HISTORY_HINT_TS = $hintKey',
      '  if ($firstSync) {',
      '    return',
      '  }',
      '  if (-not ($hintCmd.StartsWith("prun") -or $hintCmd.StartsWith("pfind") -or $hintCmd -eq "pi" -or $hintCmd.StartsWith("pi ") -or $hintCmd -eq "pil" -or $hintCmd.StartsWith("pil ") -or $hintCmd.StartsWith("npm install") -or $hintCmd.StartsWith("pnpm add") -or $hintCmd.StartsWith("pnpm install") -or $hintCmd.StartsWith("yarn add") -or $hintCmd.StartsWith("yarn install") -or $hintCmd.StartsWith("bun add") -or $hintCmd.StartsWith("bun install"))) {',
      '    return',
      '  }',
